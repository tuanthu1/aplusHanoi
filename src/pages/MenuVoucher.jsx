...
const MenuVoucher = () => {
  ...
  const handleUpdateCart = (item, amount, option = null) => {
    setCart(prev => {
      // Logic lấy tên món theo ngôn ngữ
      ...
      <div className="menu-header">
        ...
        {bookingInfo && (
          <p style={{ color: '#fff', fontSize: '16px', background: '#222', padding: '10px', borderRadius: '8px', display: 'inline-block' }}>
            {t('menu_table')}: <strong>{t('menu_voucher')}</strong> | {t('menu_guest')}: <strong>{bookingInfo.customerName}</strong>
          </p>
        )}
      </div>

      <div className="menu-tabs">
        <button className={activeTab === 'GAvoucher' ? 'active' : ''} onClick={() => setActiveTab('GAvoucher')}>GA VOUCHER</button>
      </div>
